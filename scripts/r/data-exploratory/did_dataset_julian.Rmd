...
```{r, include=FALSE}
# Load review data
did.reviews <- read.csv("~/Dropbox/Eugenie/data/processed_julian_amazon_data/did_reviews_mcauley.csv")
```
...
reviews <- reviews[reviews$date<'2017-04-02',]
```
...
# Get products
```{r, include=FALSE}
did.products <- did.reviews[,c('item_id','category','week_n','week_rating','week_word_count','week_helpful_yes','week_image_count','week','after_ban','brand', 'amazon','price')]
...
```{r, include=FALSE}
# Write to save
write.csv(did.products, file="did_products_mcauley.csv", row.names=FALSE)
```
