...
ggplot(reviews2.sentiment.all, aes(as.factor(rating), afinn.index))+
  geom_boxplot()


## boxplot bing vs. rating
## less spread out than expected, but the trend is preserved
ggplot(reviews2.sentiment.all, aes(as.factor(rating), bing.sentiment))+
  geom_boxplot()

ggplot(reviews2.sentiment.all, aes(as.factor(rating), bing.index))+
  geom_boxplot()


## boxplot bing vs. rating
## less spread out than expected, but the trend is preserved
ggplot(reviews2.sentiment.all, aes(as.factor(rating), loughran.sentiment))+
  geom_boxplot()

ggplot(reviews2.sentiment.all, aes(as.factor(rating), loughran.index))+
  geom_boxplot()


############ linear fixed effect model for reviews2.sentiment.all #############
...
model.fe <- plm(data = reviews2.sentiment.all, formula = formula.fe, index = c('item_id.x'), model = 'within')
