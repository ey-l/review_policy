...
def get_products_stats(fp, save_to, verbose=True):
	...
	# Computed total review count for each product
	...
	# Computed day count for each product
	days = groups['reviewTime'].nunique()
	df_days = days.to_frame().reset_index().rename(columns={'reviewTime':'day_count'})

	# Obtained the first reviewTime for each product
	earlist = groups['reviewTime'].min()
	df_earlist = earlist.to_frame().reset_index().rename(columns={'reviewTime':'first_reviewTime'})

	# Obtained the last reviewTime for each product
	latest = groups['reviewTime'].max()
	df_latest = latest.to_frame().reset_index().rename(columns={'reviewTime':'last_reviewTime'})

	# Computed average word count for each product
	df_length = groups['word_count'].mean().to_frame().reset_index().rename(columns={'word_count':'avg_word_count_product'})

	# Computed standar deviation of word count for each product
	df_length_std = groups['word_count'].std().to_frame().reset_index().rename(columns={'word_count':'std_word_count_product'})
	...
	# Counted reviews that exceed 2std threshold for each product
...
def get_product_burstiness_stats(fp, products_stats, save_to, verbose=True):
	...
	# Computed daily review count for each product
	df_counts = groups['overall'].count().to_frame().reset_index().rename(columns={'overall':'daily_review_count_product'})
	...
	# Computed avg review count for each product
	df_product_mean = groups['daily_review_count_product'].mean().to_frame().reset_index().rename(columns={'daily_review_count_product':'avg_review_count_product'})

	# Computed review count std for each product
	df_product_std = groups['daily_review_count_product'].std().to_frame().reset_index().rename(columns={'daily_review_count_product':'std_review_count_product'})
...
def get_category_stats(fp, save_to, verbose=True):
	'''
	Get stats for each category and save selected columns
	:param fp: file path to read from
	:param save_to: the file path saved to
	'''
	df = pd.read_csv(fp, low_memory=False)
	...
	# Computed average word count for each category
	df_length = groups['word_count'].mean().to_frame().reset_index().rename(columns={'word_count':'avg_word_count_category'})

	# Computed standar deviation of word count for each category
	df_length_std = groups['word_count'].std().to_frame().reset_index().rename(columns={'word_count':'std_word_count_category'})

	# Merged category-level statistics computed above
...
def get_category_burstiness_stats(fp, products_stats, save_to, verbose=True):
	...
	# Computed daily review count for each category
	df_counts = groups['overall'].count().to_frame().reset_index().rename(columns={'overall':'daily_review_count_category'})
	...
	# Computed daily review count for each product
	df_counts_product = groups['overall'].count().to_frame().reset_index().rename(columns={'overall':'daily_review_count_product'})
	...
	# Counted reviews that exceed 2std threshold for each product
	df_category_2std = groups['daily_review_count_2std_category'].sum().to_frame().reset_index().rename(columns={'daily_review_count_2std_category':'review_count_2std_n_category'})
